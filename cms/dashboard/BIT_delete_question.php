...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Delete query
    $query = "DELETE FROM bit_question WHERE id = ?";
    ...
    if ($stmt->execute()) {
        ...
        header("Location: BIT_Question_view.php?semester=" . urlencode($semester) . "&status=success&message=" . urlencode("Record deleted successfully."));
    } else {
        // Redirect back with an error message
        header("Location: BIT_Question_view.php?semester=" . urlencode($semester) . "&status=error&message=" . urlencode("Failed to delete the record."));
    }
