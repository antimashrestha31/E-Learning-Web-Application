...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Delete query
    $query = "DELETE FROM be_com_note WHERE id = ?";
    ...
    if ($stmt->execute()) {
        ...
        header("Location: Com_Note_view.php?semester=" . urlencode($semester) . "&status=success&message=" . urlencode("Record deleted successfully."));
    } else {
        // Redirect back with an error message
        header("Location: Com_Note_view.php?semester=" . urlencode($semester) . "&status=error&message=" . urlencode("Failed to delete the record."));
    }
